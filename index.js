...
async function getTrendingNarratives() {
  ...
  const prompt = `Today is ${today}. You are a crypto narrative analyst. Analyze these real headlines and Reddit posts from the last 24-48 hours and identify the TOP 5 emerging or accelerating crypto narratives. A narrative is an IDEA or THEME spreading through CT — not just a coin. Examples: "AI agents that hold wallets", "Bitcoin as corporate treasury reserve", "RWA tokenization going mainstream". Return ONLY a valid JSON array, no markdown, no code blocks. Only double quotes. No apostrophes. No newlines inside strings. Each narrative object has: name (short punchy name), summary (2 sentences max no apostrophes), signal_strength (1-10 based on how many sources mention it), velocity (accelerating or steady or cooling), evidence (array of max 3 specific headline snippets confirming this narrative no apostrophes), why_now (what triggered this RIGHT NOW no apostrophes), cycle_stage (early or mid-cycle or peak hype or late cooling), plays (array of 1-3 objects each with: token_name (the token or project name e.g. Ethereum VIRTUAL Chainlink), ticker (symbol e.g. ETH VIRTUAL LINK), reason (one sentence why this token is the clearest way to play this narrative no apostrophes)). Only include plays that are genuinely the clearest expression of the narrative. Sort by signal_strength descending. Only include narratives with genuine signal from the data. HEADLINES AND POSTS:\n${allContent.slice(0, 60).join('\n')}`;
